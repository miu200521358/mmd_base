...
class MFilePickerCtrl(Generic[TBaseHashModel, TBaseReader]):
    ...
    def _initialize_ui(self, name_spacer: int, tooltip: str) -> None:
        ...
        self.title_ctrl = wx.StaticText(self.parent, wx.ID_ANY, self.title, wx.DefaultPosition, wx.Size(len(self.title) * 4, -1), 0)
        ...
        if self.is_show_name and not self.is_save:
            if name_spacer:
                self.spacer_ctrl = wx.StaticText(
                    self.parent, wx.ID_ANY, " " * name_spacer, wx.DefaultPosition, wx.Size(name_spacer * 4, -1), 0
                )
