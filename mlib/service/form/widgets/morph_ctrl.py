...
class MorphChoiceCtrl:
    def __init__(
        ...
        self.left_btn_ctrl.SetToolTip(__(f"{target_name}のプルダウンの選択肢を上方向に移動できます。"))
        self.left_btn_ctrl.Bind(wx.EVT_BUTTON, self.on_change_morph_left)
        self.sizer.Add(self.left_btn_ctrl, 0, wx.ALL, 3)

        self.morph_choice_ctrl = wx.Choice(self.window, wx.ID_ANY, wx.DefaultPosition, wx.Size(choice_width, -1), choices=[])
        self.morph_choice_ctrl.SetToolTip(__(f"調整対象となる{target_name}の選択肢\n{choice_tooltip}"))
        self.morph_choice_ctrl.Bind(wx.EVT_CHOICE, self.on_change_morph)
        self.sizer.Add(self.morph_choice_ctrl, 1, wx.ALL, 3)
        ...
        self.right_btn_ctrl.SetToolTip(__(f"{target_name}のプルダウンの選択肢を下方向に移動できます。"))
    ...
    def on_change_morph_right(self, event: wx.Event) -> None:
        ...
        if selection == len(self.morph_choice_ctrl.Items) - 1:
            selection = -1
    ...
    def on_change_morph_left(self, event: wx.Event) -> None:
        selection = self.morph_choice_ctrl.GetSelection()
        if selection == 0:
            selection = len(self.morph_choice_ctrl.Items)
        self.morph_choice_ctrl.SetSelection(selection - 1)
        if self.choice_event:
            self.choice_event(event)
