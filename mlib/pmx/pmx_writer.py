...
class PmxWriter(BaseModel):
    ...
    def write_vertices(self, fout: BufferedWriter, bone_idx_type: PmxBinaryType):
        ...
        for vertex in self.model.vertices:
            ...
            if type(vertex.deform) is Bdef1:
                ...
                self.write_number(fout, PmxBinaryType.FLOAT, vertex.deform.weights[0], is_positive_only=True)
                ...
                self.write_number(fout, PmxBinaryType.FLOAT, vertex.deform.weights[0], is_positive_only=True)
                self.write_number(fout, PmxBinaryType.FLOAT, vertex.deform.weights[1], is_positive_only=True)
                self.write_number(fout, PmxBinaryType.FLOAT, vertex.deform.weights[2], is_positive_only=True)
                self.write_number(fout, PmxBinaryType.FLOAT, vertex.deform.weights[3], is_positive_only=True)
            ...
            self.write_number(fout, PmxBinaryType.FLOAT, float(vertex.edge_factor), is_positive_only=True)
    ...
    def write_materials(self, fout: BufferedWriter, texture_idx_type: PmxBinaryType):
        ...
        for midx, material in enumerate(self.model.materials):
            ...
            # Diffuse
            self.write_number(fout, PmxBinaryType.FLOAT, float(material.diffuse_color.x), is_positive_only=True)
            self.write_number(fout, PmxBinaryType.FLOAT, float(material.diffuse_color.y), is_positive_only=True)
            self.write_number(fout, PmxBinaryType.FLOAT, float(material.diffuse_color.z), is_positive_only=True)
            self.write_number(fout, PmxBinaryType.FLOAT, float(material.diffuse_color.w), is_positive_only=True)
            # Specular
            self.write_number(fout, PmxBinaryType.FLOAT, float(material.specular_color.x), is_positive_only=True)
            self.write_number(fout, PmxBinaryType.FLOAT, float(material.specular_color.y), is_positive_only=True)
            self.write_number(fout, PmxBinaryType.FLOAT, float(material.specular_color.z), is_positive_only=True)
            # Specular係数
            self.write_number(fout, PmxBinaryType.FLOAT, float(material.specular_factor), is_positive_only=True)
            # Ambient
            self.write_number(fout, PmxBinaryType.FLOAT, float(material.ambient_color.x), is_positive_only=True)
            self.write_number(fout, PmxBinaryType.FLOAT, float(material.ambient_color.y), is_positive_only=True)
            self.write_number(fout, PmxBinaryType.FLOAT, float(material.ambient_color.z), is_positive_only=True)
            # 描画フラグ(8bit)
            self.write_byte(fout, material.draw_flg.value)
            # エッジ色 (R,G,B,A)
            self.write_number(fout, PmxBinaryType.FLOAT, float(material.edge_color.x), is_positive_only=True)
            self.write_number(fout, PmxBinaryType.FLOAT, float(material.edge_color.y), is_positive_only=True)
            self.write_number(fout, PmxBinaryType.FLOAT, float(material.edge_color.z), is_positive_only=True)
            self.write_number(fout, PmxBinaryType.FLOAT, float(material.edge_color.w), is_positive_only=True)
            # エッジサイズ
            self.write_number(fout, PmxBinaryType.FLOAT, float(material.edge_size), is_positive_only=True)
            # 通常テクスチャ
            ...
            if bone.is_ik():
                ...
                for link in bone.ik.links:
                    ...
                    if link.angle_limit == 1:
                        self.write_number(fout, PmxBinaryType.FLOAT, float(link.min_angle_limit.radians.x))
                        self.write_number(fout, PmxBinaryType.FLOAT, float(link.min_angle_limit.radians.y))
                        self.write_number(fout, PmxBinaryType.FLOAT, float(link.min_angle_limit.radians.z))

                        self.write_number(fout, PmxBinaryType.FLOAT, float(link.max_angle_limit.radians.x))
                        self.write_number(fout, PmxBinaryType.FLOAT, float(link.max_angle_limit.radians.y))
                        self.write_number(fout, PmxBinaryType.FLOAT, float(link.max_angle_limit.radians.z))
    ...
    def write_morphs(
        ...
        """
        # モーフの数
        self.write_number(fout, PmxBinaryType.INT, len(self.model.morphs), is_positive_only=True)

        for midx, morph in enumerate(self.model.morphs):
            # モーフ名
    ...
    def write_display_slots(self, fout: BufferedWriter, bone_idx_type: PmxBinaryType, morph_idx_type: PmxBinaryType):
        """
        表示枠出力
        ...
        # 表示枠の数
        self.write_number(fout, PmxBinaryType.INT, len(self.model.display_slots), is_positive_only=True)
    ...
    def write_rigidbodies(self, fout: BufferedWriter, bone_idx_type: PmxBinaryType):
        ...
        for ridx, rigidbody in enumerate(self.model.rigidbodies):
            ...
            # 12 : float3	| サイズ(x,y,z)
            self.write_number(fout, PmxBinaryType.FLOAT, float(rigidbody.shape_size.x), is_positive_only=True)
            self.write_number(fout, PmxBinaryType.FLOAT, float(rigidbody.shape_size.y), is_positive_only=True)
            self.write_number(fout, PmxBinaryType.FLOAT, float(rigidbody.shape_size.z), is_positive_only=True)
            # 12 : float3	| 位置(x,y,z)
            ...
            # 4  : float	| 質量
            self.write_number(fout, PmxBinaryType.FLOAT, float(rigidbody.param.mass), is_positive_only=True)
            # 4  : float	| 移動減衰
            self.write_number(fout, PmxBinaryType.FLOAT, float(rigidbody.param.linear_damping), is_positive_only=True)
            # 4  : float	| 回転減衰
            self.write_number(fout, PmxBinaryType.FLOAT, float(rigidbody.param.angular_damping), is_positive_only=True)
            # 4  : float	| 反発力
            self.write_number(fout, PmxBinaryType.FLOAT, float(rigidbody.param.restitution), is_positive_only=True)
            # 4  : float	| 摩擦力
            self.write_number(fout, PmxBinaryType.FLOAT, float(rigidbody.param.friction), is_positive_only=True)
    ...
    def write_joints(self, fout: BufferedWriter, rigidbody_idx_type: PmxBinaryType):
        ...
        for jidx, joint in enumerate(self.model.joints):
            ...
            self.write_number(fout, PmxBinaryType.FLOAT, float(joint.param.rotation_limit_min.radians.x))
            self.write_number(fout, PmxBinaryType.FLOAT, float(joint.param.rotation_limit_min.radians.y))
            self.write_number(fout, PmxBinaryType.FLOAT, float(joint.param.rotation_limit_min.radians.z))
            # 12 : float3	| 回転制限-上限(x,y,z) -> ラジアン角
            self.write_number(fout, PmxBinaryType.FLOAT, float(joint.param.rotation_limit_max.radians.x))
            self.write_number(fout, PmxBinaryType.FLOAT, float(joint.param.rotation_limit_max.radians.y))
            self.write_number(fout, PmxBinaryType.FLOAT, float(joint.param.rotation_limit_max.radians.z))
            # 12 : float3	| バネ定数-移動(x,y,z)
            self.write_number(fout, PmxBinaryType.FLOAT, float(joint.param.spring_constant_translation.x))
            self.write_number(fout, PmxBinaryType.FLOAT, float(joint.param.spring_constant_translation.y))
            self.write_number(fout, PmxBinaryType.FLOAT, float(joint.param.spring_constant_translation.z))
    ...
    def write_text(self, fout: BufferedWriter, text: str, default_text: str, type: PmxBinaryType = PmxBinaryType.INT):
        """
        文字列出力
    ...
    def write_number(self, fout: BufferedWriter, val_type: PmxBinaryType, val: float, default_value: float = 0.0, is_positive_only: bool = False):
        """
        数値出力
