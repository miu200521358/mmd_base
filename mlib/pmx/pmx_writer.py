        ...
        with open(output_path, "wb") as fout:
            ...
            for vertex in model.vertices:
                ...
                if type(vertex.deform) is Bdef1:
                    fout.write(struct.pack(TYPE_BYTE, 0))
                    write_number(fout, bone_idx_type, vertex.deform.indices[0])
                elif type(vertex.deform) is Bdef2:
                    fout.write(struct.pack(TYPE_BYTE, 1))
                    write_number(fout, bone_idx_type, vertex.deform.indices[0])
                    write_number(fout, bone_idx_type, vertex.deform.indices[1])
                    ...
                    write_number(fout, bone_idx_type, vertex.deform.indices[0])
                    write_number(fout, bone_idx_type, vertex.deform.indices[1])
                    write_number(fout, bone_idx_type, vertex.deform.indices[2])
                    write_number(fout, bone_idx_type, vertex.deform.indices[3])
                ...
                if bone.is_ik():
                    ...
                    for link in bone.ik.links:
                        ...
                        if link.angle_limit == 1:
                            write_number(
                                fout, TYPE_FLOAT, float(link.min_angle_limit.radians.x)
                            )
                            write_number(
                                fout, TYPE_FLOAT, float(link.min_angle_limit.radians.y)
                            )
                            write_number(
                                fout, TYPE_FLOAT, float(link.min_angle_limit.radians.z)
                            )

                            write_number(
                                fout, TYPE_FLOAT, float(link.max_angle_limit.radians.x)
                            )
                            write_number(
                                fout, TYPE_FLOAT, float(link.max_angle_limit.radians.y)
                            )
                            write_number(
                                fout, TYPE_FLOAT, float(link.max_angle_limit.radians.z)
                            )
            ...
            for midx, morph in enumerate(model.morphs):
                ...
                for offset in morph.offsets:
                    if type(offset) is VertexMorphOffset:
                        ...
                        # 材質モーフ
                        fout.write(
                            struct.pack(material_idx_type, offset.material_index)
                        )
                        ...
                        write_number(
                            fout, TYPE_FLOAT, float(offset.sphere_texture_factor.x)
                        )
                        write_number(
                            fout, TYPE_FLOAT, float(offset.sphere_texture_factor.y)
                        )
                        write_number(
                            fout, TYPE_FLOAT, float(offset.sphere_texture_factor.z)
                        )
                        write_number(
                            fout, TYPE_FLOAT, float(offset.sphere_texture_factor.w)
                        )
                        write_number(
                            fout, TYPE_FLOAT, float(offset.toon_texture_factor.x)
                        )
                        write_number(
                            fout, TYPE_FLOAT, float(offset.toon_texture_factor.y)
                        )
                        write_number(
                            fout, TYPE_FLOAT, float(offset.toon_texture_factor.z)
                        )
                        write_number(
                            fout, TYPE_FLOAT, float(offset.toon_texture_factor.w)
                        )
            ...
            for ridx, rigidbody in enumerate(model.rigidbodies):
                ...
                # 12 : float3	| 回転(x,y,z)
                write_number(
                    fout, TYPE_FLOAT, float(rigidbody.shape_rotation.radians.x)
                )
                write_number(
                    fout, TYPE_FLOAT, float(rigidbody.shape_rotation.radians.y)
                )
                write_number(
                    fout, TYPE_FLOAT, float(rigidbody.shape_rotation.radians.z)
                )
                # 4  : float	| 質量
                write_number(fout, TYPE_FLOAT, float(rigidbody.param.mass), True)
                # 4  : float	| 移動減衰
                write_number(
                    fout, TYPE_FLOAT, float(rigidbody.param.linear_damping), True
                )
                # 4  : float	| 回転減衰
                write_number(
                    fout, TYPE_FLOAT, float(rigidbody.param.angular_damping), True
                )
                # 4  : float	| 反発力
            ...
            for jidx, joint in enumerate(model.joints):
                ...
                write_number(
                    fout, TYPE_FLOAT, float(joint.param.translation_limit_min.y)
                )
                write_number(
                    fout, TYPE_FLOAT, float(joint.param.translation_limit_min.z)
                )
                # 12 : float3	| 移動制限-上限(x,y,z)
                write_number(
                    fout, TYPE_FLOAT, float(joint.param.translation_limit_max.x)
                )
                write_number(
                    fout, TYPE_FLOAT, float(joint.param.translation_limit_max.y)
                )
                write_number(
                    fout, TYPE_FLOAT, float(joint.param.translation_limit_max.z)
                )
                # 12 : float3	| 回転制限-下限(x,y,z) -> ラジアン角
                write_number(
                    fout, TYPE_FLOAT, float(joint.param.rotation_limit_min.radians.x)
                )
                write_number(
                    fout, TYPE_FLOAT, float(joint.param.rotation_limit_min.radians.y)
                )
                write_number(
                    fout, TYPE_FLOAT, float(joint.param.rotation_limit_min.radians.z)
                )
                # 12 : float3	| 回転制限-上限(x,y,z) -> ラジアン角
                write_number(
                    fout, TYPE_FLOAT, float(joint.param.rotation_limit_max.radians.x)
                )
                write_number(
                    fout, TYPE_FLOAT, float(joint.param.rotation_limit_max.radians.y)
                )
                write_number(
                    fout, TYPE_FLOAT, float(joint.param.rotation_limit_max.radians.z)
                )
                # 12 : float3	| バネ定数-移動(x,y,z)
                write_number(
                    fout, TYPE_FLOAT, float(joint.param.spring_constant_translation.x)
                )
                write_number(
                    fout, TYPE_FLOAT, float(joint.param.spring_constant_translation.y)
                )
                write_number(
                    fout, TYPE_FLOAT, float(joint.param.spring_constant_translation.z)
                )
                # 12 : float3	| バネ定数-回転(x,y,z)
                write_number(
                    fout, TYPE_FLOAT, float(joint.param.spring_constant_rotation.x)
                )
                write_number(
                    fout, TYPE_FLOAT, float(joint.param.spring_constant_rotation.y)
                )
                write_number(
                    fout, TYPE_FLOAT, float(joint.param.spring_constant_rotation.z)
                )
...
def write_number(fout, val_type: str, val: float, is_positive_only=False):
    ...
    try:
        ...
        logger.error(
            "write_number失敗: type: %s, val: %s, int(val): %s", val_type, val, int(val)
        )
        raise e
