...
void main() {
    ...
    // 頂点法線
    vec3 N = (modelMatrix * normalize(vec4(normal, 1.0))).xyz;
    ...
    vertexColor.xyz += diffuse.xyz * factor;

    // saturate
    vertexColor = clamp(vertexColor, 0.0, 1.0);

    // カメラとの相対位置
    vec3 eye = cameraPos - gl_Position.xyz;

    // スペキュラ色計算
    vec3 HalfVector = normalize( normalize(eye) + LightDirection );
    vertexSpecular = clamp(pow( max(0, dot( HalfVector, vetexNormal )), specular.w ) * specular.xyz, 0.0f, 1.0f);
}
""" % (
    POSITION_ID,
    NORMAL_ID,
