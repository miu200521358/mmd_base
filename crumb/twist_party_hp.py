...
# 出力用のDataFrameを初期化
output_columns = [
    "カード1", "カード2", "カード3", "カード4", "カード5", "HP合計", "ATK合計", "火属性数", "水属性数", "木属性数", "無属性数",
    "デュオ数", "デュオ1", "デュオ2", "デュオ3", "デュオ4", "デュオ5",
    "HP UP(中)数", "HP UP(中)1", "HP UP(中)2", "HP UP(中)3", "HP UP(中)4", "HP UP(中)5", "HP UP(中)6",
    "HP UP(小)数", "HP UP(小)1", "HP UP(小)2", "HP UP(小)3", "HP UP(小)4", "HP UP(小)5", "HP UP(小)6",
    "ATK UP(中)数", "ATK UP(中)1", "ATK UP(中)2", "ATK UP(中)3", "ATK UP(中)4", "ATK UP(中)5", "ATK UP(中)6",
    "ATK UP(小)数", "ATK UP(小)1", "ATK UP(小)2", "ATK UP(小)3", "ATK UP(小)4", "ATK UP(小)5", "ATK UP(小)6"
]
...
def check_conditions(cards):
    ...
    # ATK合計のチェック
    if cards['ATK'].sum() < 23000:
        return False

    # HP合計のチェック
    if cards['HP'].sum() < 55000:
        return False

    # 該当する編成におけるデュオ魔法、HP UP(中)、HP UP(小)の数を計算
    counts = {
        'デュオ魔法': 0,
        'HP UP(中)': 0,
        'HP UP(小)': 0
    ...
    for k in range(1, 4):
        ...
        counts['HP UP(中)'] += sum((cards[buddy_skill] == 'HP UP(中)') & (cards[buddy_skill_partner].isin(cards['カードキャラクター名'])))
        counts['HP UP(小)'] += sum((cards[buddy_skill] == 'HP UP(小)') & (cards[buddy_skill_partner].isin(cards['カードキャラクター名'])))

    # 条件を満たしているかチェック
    if (counts['HP UP(中)'] + (counts['HP UP(小)'] / 2)) >= 3:
        return True
...
def add_to_output_df(selected_cards, output_df):
    # デュオ魔法、HP UP(中)、HP UP(小)の組み合わせを探索
    duos = []
    hp_up_medium = []
    hp_up_small = []
    atk_up_medium = []
    ...
    for i in range(5):
        ...
        for j in range(5):
            ...
            if card_i['デュオ魔法相手'] == card_j['カードキャラクター名']:
                duos.append(f"{card_i['カードキャラクター名']}-{card_i['カード種類']}&{card_j['カードキャラクター名']}-{card_j['カード種類']}")
            ...
            for k in range(1, 4):
                ...
                if card_i[buddy_skill_partner] == card_j['カードキャラクター名']:
                    if card_i[buddy_skill] == 'HP UP(中)':
                        hp_up_medium.append(f"{card_i['カードキャラクター名']}-{card_i['カード種類']}&{card_j['カードキャラクター名']}-{card_j['カード種類']}")
                    elif card_i[buddy_skill] == 'HP UP(小)':
                        hp_up_small.append(f"{card_i['カードキャラクター名']}-{card_i['カード種類']}&{card_j['カードキャラクター名']}-{card_j['カード種類']}")
                    elif card_i[buddy_skill] == 'ATK UP(中)':
                        atk_up_medium.append(f"{card_i['カードキャラクター名']}-{card_i['カード種類']}&{card_j['カードキャラクター名']}-{card_j['カード種類']}")
                    elif card_i[buddy_skill] == 'ATK UP(小)':
                        atk_up_small.append(f"{card_i['カードキャラクター名']}-{card_i['カード種類']}&{card_j['カードキャラクター名']}-{card_j['カード種類']}")
...
# キャラ名一覧
character_names = ["リドル", "エース", "デュース", "トレイ", "ケイト", "レオナ", "ラギー", "ジャック", "アズール", "ジェイド", "フロイド", "カリム", "ジャミル", "ヴィル", "ルーク", "エペル", "イデア", "オルト", "マレウス", "リリア", "シルバー", "セベク"]

# メインカードのキャラクター名以外のキャラ名を取得
other_character_names = [character_name for character_name in character_names if character_name != main_card_character]
...
for n, other_characters in enumerate(tqdm(other_character_combinations)):
    ...
    for other_character in other_characters:
        ...
        other_character_cards = ssr_card_df[(ssr_card_df['カードキャラクター名'] == other_character)]
        card_combinations.append(other_character_cards.values.tolist())

    # 5名のカード一覧を組み合わせる
    character_products = product(card_combinations[0], card_combinations[1], card_combinations[2], card_combinations[3], card_combinations[4])

    for i, character_product in enumerate(character_products):
        cards = pd.DataFrame(character_product, columns=['カードキャラクター名', 'カード種類', 'カード属性', '魔法1属性', '魔法2属性', 'HP', 'ATK', 'デュオ魔法相手', 'バディスキル1相手', 'バディスキル1', 'バディスキル2相手', 'バディスキル2', 'バディスキル3相手', 'バディスキル3'])
...
output_df = output_df.sort_values(by=["HP合計", "HP UP(中)数", "HP UP(小)数", "デュオ数"], ascending=[False, False, False, False])
