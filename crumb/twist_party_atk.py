...
def check_conditions(cards):
    ...
    for k in range(1, 4):
        ...
        counts['ATK UP(中)'] += sum((cards[buddy_skill] == 'ATK UP(中)') & (cards[buddy_skill_partner].isin(cards['カードキャラクター名'])))
        counts['ATK UP(小)'] += sum((cards[buddy_skill] == 'ATK UP(小)') & (cards[buddy_skill_partner].isin(cards['カードキャラクター名'])))
...
output_df = output_df.sort_values(by=["デュオ数", "ATK合計", "ATK UP(中)数", "ATK UP(小)数"], ascending=[False, False, False, False])
